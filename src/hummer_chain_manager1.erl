...
cl_write_public_proj2(FLUs, Partitions, Epoch, Proj, IgnoreWrittenErrorP, S) ->
    ...
    DoIt = fun(Pid) -> try ?FLU_PC:write_projection(Pid, public, Proj, ?TO) catch _:_ -> {error, partition} end end,
                   ...
                   case perhaps_call_t(S, Partitions, FLU, DoIt) of
                       ...
                       {error,written}=Written when not IgnoreWrittenErrorP ->
                           ...
                           {ok, Proj2} = perhaps_call_t(S, Partitions, FLU,
                                                        DoRead),
                           DoIt2 = fun(Pid) -> try ?FLU_PC:write_projection(Pid, public, Proj2, ?TO)  catch _:_ -> {error, partition} end end,
...
calc_projection(#ch_mgr{name=_MyName, consistency_mode=CMode,
    ...
    if CMode == ap_mode ->
            A = calc_projection2(P_current, RelativeToServer, AllHosed, Dbg, S),
            io:format("A ~p~n", [A]),
            A;
       CMode == cp_mode ->
...
do_set_chain_members_dict(MembersDict, #ch_mgr{proxies_dict=OldProxiesDict}=S)->
    ...
    ProxiesDict = dict:new(), %?FLU_PC:start_proxies(MembersDict),
